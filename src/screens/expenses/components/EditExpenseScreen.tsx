...
export function EditExpenseScreen({
  ...
  // Initialize form when expense updates
  useEffect(() => {
    if (expense) {
      ...
      // Parse status out of description
      const descRaw = expense.description || '';
      const statusMatch = descRaw.match(/^\[STATUS:(\w+)\]\s*/);
      if (statusMatch) {
        setStatus(statusMatch[1] as any);
        setDescription(descRaw.replace(/^\[STATUS:\w+\]\s*/, ''));
      } else {
        setStatus('paid');
        setDescription(descRaw);
      }
    }
  }, [expense]);
