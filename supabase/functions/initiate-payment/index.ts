...
function validateRequest(body: InitiatePaymentRequest): string | null {
  ...
  if (!body.idempotency_key)   return 'idempotency_key is required';

  if (body.payment_type !== 'payout') {
    if (!body.payer_phone) return 'payer_phone is required for collections';
  } else {
    if (!body.recipient_name) return 'recipient_name is required for payouts';
    if (body.channel === 'bank') {
      if (!body.recipient_bank)    return 'recipient_bank is required for bank payout';
      if (!body.recipient_account) return 'recipient_account is required for bank payout';
    } else {
      if (!body.recipient_phone) return 'recipient_phone is required for mobile payout';
    }
  }
...
async function resolveConfig(
  ...
  const webhookUrl = gatewayCfg?.webhook_url as string | undefined;
